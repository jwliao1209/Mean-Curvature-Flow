% main script for color image
clear; clc;
O = imread('majo.jpg'); Type = 'original';
O = double(O) / 255;
noise_sigma = 0.08;
...
II = zeros(size(I)); III = II; IV = II;
% For RGB channel
for ii = 1 : 3
    II(:,:,ii) = Mean_Curvature_Equation(I(:,:,ii), iter(1), jmp, dist, w, sigma);
    III(:,:,ii) = Mean_Curvature_Equation(I(:,:,ii), iter(2), jmp, dist, w, sigma);
    IV(:,:,ii) = Mean_Curvature_Equation(I(:,:,ii), iter(3), jmp, dist, w, sigma);
end
